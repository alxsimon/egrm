...
# exports [matrix._matrix_C_API] object into a 2d numpy array mat_C:
# [matrix._matrix_C_API] object initiated by matrix.new_matrix,
# only stores the upper triangle elements of a square matrix N:
...
# defines [Gmap] object which maps the physical position (in bp) into genetic
# position (in unit of 10^-6 cM) can be initiated by Gmap(filename),
# where filename is a (comma/space/tab separated) three-column file
# with first column specifying the physical position in bp and the third column
# specifying the genetic position in cM.
# The second column is not used.
# The first line will always be ignored as the header.
class Gmap:
    ...
    def __call__(self, pos):
        ...
        B = (self.gpos[self.i] - self.gpos[self.i - 1]) / (
            self.pos[self.i] - self.pos[self.i - 1]
        )
...
def varGRM_C(
    ...
    for tree in trees_:
        pbar.update(1)
        region_length = -gmap(max(left, tree.interval[0])) + gmap(
            min(right, tree.interval[1])
        )
        if region_length <= 0:
            continue
        ...
        for c in tree.nodes():
            ...
            if var:
                matrix.add_square(
                    egrm2_C,
                    descendants,
                    mu * np.power(g(p), 2) * np.power((1 - 2 * p), 2),
                )
                tmp1[descendants] += (
                    mu * np.power(g(p), 2) * (np.power(p, 2) - 2 * np.power(p, 3))
                )
    ...
    if var:
        e = np.reciprocal(
            (lambda x: x[x != 0])(np.random.poisson(lam=total_mu, size=10000)).astype(
                "float"
            )
        ).mean()
        vargrm = e * (
...
# gmap: Gmap object that maps the physical position (in unit of base pair) into genetic position (in unit of 10^-6 centimorgan).
# sft: True/False variable indicating whether the first tree is to be skipped. Not recommended to use together with [left] and [right] options.
def mTMRCA_C(trees, log=None, left=0, right=math.inf, gmap=Gmap(None), sft=False):
    N = trees.num_samples
    ...
    for tree in trees_:
        ...
        for c in tree.nodes():
            ...
            matrix.add_square(mtmrca_C, descendants, t * region_length)
        tmp += height * region_length
        total_l += region_length
...
def varGRM(
    ...
    for tree in trees_:
        ...
        for c in tree.nodes():
            ...
            if var:
                egrm2[np.ix_(descendants, descendants)] += (
                    mu * np.power(g(p), 2) * np.power((1 - 2 * p), 2)
                )
                tmp1[descendants] += (
                    mu * np.power(g(p), 2) * (np.power(p, 2) - 2 * np.power(p, 3))
...
def mTMRCA(trees, log=None, left=0, right=math.inf, gmap=Gmap(None), sft=False):
    N = trees.num_samples
    mtmrca = np.zeros([N, N])
    tmp = 0
    total_l = 0
    ...
    for tree in trees_:
        ...
        for c in tree.nodes():
            ...
            t = tree.time(tree.parent(c)) - tree.time(c)
            height = max(height, tree.time(tree.parent(c)))
            mtmrca[np.ix_(descendants, descendants)] += t * region_length
        tmp += height * region_length
        total_l += region_length
